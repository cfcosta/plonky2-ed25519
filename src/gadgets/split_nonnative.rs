...
pub trait CircuitBuilderSplit<F: RichField + Extendable<D>, const D: usize> {
    ...
    fn split_nonnative_to_4_bit_limbs<FF: Field>(&mut self, val: &NonNativeTarget<FF>)
        -> Vec<Target>;

    fn split_nonnative_to_2_bit_limbs<FF: Field>(&mut self, val: &NonNativeTarget<FF>)
        -> Vec<Target>;
...
mod tests {
    ...
    fn test_split_nonnative() -> Result<()> {
        ...
        let combined: NonNativeTarget<Ed25519Scalar> = builder.recombine_nonnative_4_bit_limbs(split);
